...
import newPTCutUtils
import NEWtmva_utils
...
fin = uproot.open("/home/users/rlee/NTuples/GJets_2017UL_NoPTMCut_TMVA_0312_Train_Proc.root")
...
#dptCut = lambda tree: abs((tree.array('genPt')-tree.array('pt'))/tree.array('genPt')) < 0.05
ptCut = lambda tree: tree.array('pt') > 18.0


#input_values, target_values, orig_weights, train_weights, pt, scEta, input_vars = newPTCutUtils.load_file(fin,geometry_selection,ptCut)
input_values, target_values, orig_weights, train_weights, pt, scEta, input_vars = newPTCutUtils.load_file(fin,geometry_selection,ptCut)
...
X_train, X_test, w_train, dummy, y_train, y_test, dummy, w_test, pt_train, pt_test, scEta_train, scEta_test = train_test_split(input_values,train_weights,target_values,orig_weights,pt,scEta,test_size=0.50)
...
model_name = "Model_0312_M18_LR03_GJets_UL2017_NoPTMCut"
...
NEWtmva_utils.convert_model(model.get_booster().get_dump(), input_variables = input_variables, output_xml=model_xmlName)
...
NEWtmva_utils.variable_importance(model,input_variables,model_name)
