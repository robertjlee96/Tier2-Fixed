...
<<<<<<< HEAD
    fileNames[0] = "../NTuples/GGH_And_GJets_TMVA_1206.root";
    fileNames[1] = "../NTuples/GGH_And_GJets_Prasant_0211.root";
    fileNames[2] = "../NTuples/GGH_And_GJets_M95PTM15_MD20LR045_DPT075_0202.root";
...
>>>>>>> 27f7c4344d75d61327757beea580fc85cb3f969b
    ...
    string genLabels[10];
<<<<<<< HEAD
    genLabels[0] = " || TRAIN:TMVA";
    genLabels[1] = " || TRAIN:Prasant XGBoost";
    genLabels[2] = " || TRAIN Robert XGBoost";// M95, PTM15, DPT075, MD20 LR0.045";
    ...
    string mvaOut = "curves/"+ etaRegion + "Photons_BestVsTMVAandPrasant_PreselDen_GGH_0212";
=======
...
>>>>>>> 27f7c4344d75d61327757beea580fc85cb3f969b
    ...
    TLegend *legend_RoC = new TLegend(0.2,0.1,0.9,0.65,"","brNDC");
    ...
    else if (etaRegion == "Barrel-Barrel" || etaRegion == "barrel-barrel") {
        etaCutLead = " && abs(leadScEta) < 1.4442 && abs(subScEta) < 1.4442";
        etaCutSub = " && abs(leadScEta) < 1.4442 && abs(subScEta) < 1.4442";
        etaRegionStr = "Barrel-Barrel ";
    }
    else if(etaRegion == "Endcap-Endcap" || etaRegion == "endcap-endcap"){
        etaCutLead = " && abs(leadScEta) > 1.556 && abs(subScEta) > 1.556";
        etaCutSub = " && abs(leadScEta) > 1.556 && abs(subScEta) > 1.556";
        etaRegionStr = "Endcap-Endcap ";
    }
    else if(etaRegion == "Mixed" || etaRegion == "mixed"){
        etaCutLead = " && ((abs(leadScEta) > 1.556 && abs(subScEta) < 1.4442) || (abs(leadScEta) < 1.4442 && abs(subScEta) > 1.556))";
        etaCutSub = " && ((abs(leadScEta) > 1.556 && abs(subScEta) < 1.4442) || (abs(leadScEta) < 1.4442 && abs(subScEta) > 1.556))";
        etaRegionStr = "Barrel-Endcap ";
    }
    else{
        etaCutLead = "";
        etaCutSub = "";
        etaRegionStr = "All";
        cout<<"Plotting for all scEta -- Unrecognized string (use 'Barrel-Barrel', 'Endcap-Endcap', 'Barrel', 'Endcap', or 'Mixed')"<<endl;
    }
    ...
    string massCutSig = " && hggMass > 120 && hggMass < 128.5";
    ...
    //for(int i = 0; i < 3; i++){
        ...
        snprintf(buff1, sizeof(buff1), "(%0.2f)", hSigLoose->GetSumOfWeights());
        string nEventsSigLoose = buff1;
        
        snprintf(buff2, sizeof(buff2), "(%0.2f)", hSigTight->GetSumOfWeights());
        string nEventsSigTight = buff2;
        
        snprintf(buff3, sizeof(buff3), "(%0.2f)", hBkgLoose->GetSumOfWeights());
        string nEventsBkgLoose = buff3;
        
        snprintf(buff4, sizeof(buff4), "(%0.2f)", hBkgTight->GetSumOfWeights());
...
//            bkgEffTight[k] = NbkgTight[k]/NbkgLoose[0];
            ...
            //FOR PRESEL DEN
            sigEffLoose[k] = NsigLoose[k]/NsigTight[0];
            sigEffTight[k] = NsigTight[k]/NsigTight[0];

            bkgEffLoose[k] = NbkgLoose[k]/NbkgTight[0];
            bkgEffTight[k] = NbkgTight[k]/NbkgTight[0];
...
//            sigEff_vs_bkgEff_Loose->GetXaxis()->SetRangeUser(0.0,0.32);
        ...
        sigEff_vs_bkgEff_Loose->SetLineWidth(1);
        sigEff_vs_bkgEff_Tight->SetLineWidth(1);
        sigEff_vs_bkgEff_Tight->SetLineStyle(10);
        
        if (i == 1){
            sigEff_vs_bkgEff_Loose->SetLineColor(kGreen+3);
            sigEff_vs_bkgEff_Loose->SetMarkerColor(kGreen+3);
            sigEff_vs_bkgEff_Tight->SetLineColor(kGreen+3);
            sigEff_vs_bkgEff_Tight->SetMarkerColor(kGreen+3);
        }
        if (i == 2){
            sigEff_vs_bkgEff_Loose->SetLineColor(2);
            sigEff_vs_bkgEff_Loose->SetMarkerColor(2);
            sigEff_vs_bkgEff_Tight->SetLineColor(2);
            sigEff_vs_bkgEff_Tight->SetMarkerColor(2);
        }
        can_RoC->SetGrid();
        
        string titleString = "RoC Curves W/ GGH Signal & GJet Fake Background  " + etaRegionStr + " Photons " + weightCutStr;
